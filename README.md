...
for stats in plain language. Full spec: Obsidian vault `Projects/MyFinanceMCP.md`.
...
```bash
cp .env.example .env   # fill MYFINANCE_MCP_EMAIL + MYFINANCE_MCP_PASSWORD_HASH
bun install
...
`https://finance.rteam.agency/mcp` -> sign in. Then ask Claude to "ping MyFinance MCP".
